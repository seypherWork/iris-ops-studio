...
| Web apps | `GET /v2/web-apps`, `GET /v2/web-app` | Maps authentication methods, dispatch class, namespace, and resource; reconciles enabled state, namespace, dispatch class, and resource with the authoritative detail endpoint, and reports `Unknown` if detail cannot be read |
