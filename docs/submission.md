# Contest submission record — version 1.2.1

This document describes the version 1.2.1 scope; the current Open Exchange
release and contest statuses must be checked on the public listing. Version
1.2.1 adds optional, bounded IRIS-native messages,
System Monitor, and alerts pages to Incident Timeline, plus guided
`EditResource`/`UseResource` editing for existing non-system wallet
collections. The fresh-state check and complete two-field readback were
exercised on disposable IRIS Community 2026.2. No secret values are read.
There is no portal-wide server-enforced safe mode, atomic wallet write,
complete user-impact graph, or journal/interoperability parser. A versioned
archive was extracted, installed and retested on disposable IRIS.
...
an Incident Timeline spanning audit/task/session evidence and optionally
three bounded native-log sources, direct SysAdmin API exploration, and
read-only REST documentation discovery.
...
Earlier version 1.2.0 added guided availability changes for eligible non-system web
...
Version 1.2.1 also adds a separate, authenticated GET-only
native-log endpoint and a policy-only wallet editor. Neither changes the
browser-side nature of the SysAdmin mutation guardrails.
...
5. Open Logs & audit and filter Incident Timeline across Audit, Tasks, Ops
   Studio, and the labeled native-log demo fixtures. In Live IRIS, show the
   optional native source-status cards and bounded pages.
...
12. In Secrets inventory, preview both fields of a disposable wallet access
    policy and show that a stale precondition sends no `PUT`.
...
- Fail-closed access updates with stale-preview blocking and exact full-set
  verification.
- Incident Timeline with audit/task/session sources and optional bounded
  native messages, System Monitor, and alerts; source, severity, free-text,
  entity, and correlation filtering.
...
- Public 1.2.0 had 84 automated tests covering the client, browser coordination, explorer, permission plans,
  ...
  are separate.
- Approximately 79.4% aggregate line coverage in the 1.2.0 run, including
...
- Version 1.2.1 passes 103 JavaScript tests (81.17%
  measured aggregate Node line coverage) and 20 POSIX reader tests on Linux.
  Its corrected versioned archive was also installed and exercised in a fresh
  disposable IRIS instance. See `docs/development-validation-20260925.md`.
...
- Version 1.2.1 passed scoped fresh-install, authenticated native-log and
  wallet-policy checks on disposable IRIS; the exact limits are in
  `docs/development-validation-20260925.md`. Verify the public release version
  on Open Exchange independently of this local evidence.
